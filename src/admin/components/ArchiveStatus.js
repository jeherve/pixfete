...
export function ArchiveStatus({ archive, dateRangeEnd }) {
	...
	if (!dateRangeEnd || !isEventEnded(dateRangeEnd)) {
		return <p>{__('The photo archive will be available once the event ends.', 'pixfete')}</p>;
	}

	// Event has ended but no archive entry exists yet (cron hasn't run).
	if (!archive) {
		return <p>{__('Photo archive is queued for generation.', 'pixfete')}</p>;
	}
	...
	switch (status) {
		...
		case 'generating':
			return (
				<p>
					{__('Photo archive is being generated…', 'pixfete')} <Spinner />
				</p>
			);
		...
		case 'complete':
			return (
				<div>
					<p>{__('Photo archive ready.', 'pixfete')}</p>
					<Button variant="secondary" href={url} download>
						{__('Download ZIP', 'pixfete')}
					</Button>
				</div>
			);

		case 'failed':
			return <p>{__('Archive generation failed.', 'pixfete')}</p>;
